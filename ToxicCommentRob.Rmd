...
### NRC
nrc.sent<-sentiment_by(cmmnt_df$comments, list(id), lexicon="nrc")
nrc.sent.t<-sentiment_by(cmmnt_df$comments, list(id), lexicon="nrc")

colnames(nrc.sent)[4] <- "NRC"
colnames(nrc.sent.t)[4]<-"NRC"

toxic_train<-cbind(toxic_train,(nrc.sent[,4]))
toxic_test<-cbind(toxic_test,(nrc.sent.t[,4]))
...
non.hate.pred<-ifelse(non.hate.prob>.1,"1","0")
...
toxic_pred<-ifelse(toxic_prob>.99,"1","0")

confusionMatrix(toxic_pred, toxic_test$toxic, positive = "1")

###severe toxic --- no good

s.toxic.lr<-glm(severe_toxic~swear_sent+ncap+ncap_len+nexcl+nquest+npunct_len+nsent+nsymb+nwords+nhate+afinn, data=toxic_train, family= binomial)
...
obscene.lr <-glm(obscene~swear_sent+hate_sent+ncap+ncap_len+nquest+nsent+nsymb+nwords+nhate+afinn, data=toxic_train, family= binomial)
...
###insult--good
insult.lr<-glm(insult~swear_sent+hate_sent+ncap_len+nquest+npunct_len+nsymb+nwords+nhate+afinn, data=toxic_train, family= binomial)
...
threat.lr<-glm(threat~swear_sent+ncap_len+npunct_len+nsent+nsymb+nwords+nhate+afinn, data=toxic_train, family=binomial)
...
hate.lr<-glm(identity_hate~ncap_len+npunct_len+nsent+nwords+nhate+afinn, data=toxic_train, family=binomial)
...
###naive bayes
```{r}
matrix<-create_matrix(toxic_train[,2],language="english",removeStopwords=TRUE,removeNumbers=FALSE,stemWords=FALSE)
mat<-as.matrix(matrix)
classifier<-naiveBayes(mat[1:20000,],toxic_train[1:20000,9:25])
...
toxic_preds<-ifelse(lasso.pred.toxic>.2,"1","0")
