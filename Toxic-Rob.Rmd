...
swear.sent.train<-sentiment_by(trainsentences,list(id), polarity_dt=mykey.swear)
swear.sent.test<-sentiment_by(testsentenses,list(id),polarity_dt=mykey.swear)

swear.sent.test
colnames(swear.sent.test)[4] <- "swear_sent"
colnames(swear.sent.train)[4]<-"swear_sent"
key.hatewords <- key.hatewords[-c(1014), ]
hate.sent.train<-sentiment_by(trainsentences,list(id), polarity_dt=key.hatewords)
hate.sent.test<-sentiment_by(testsentenses,list(id),polarity_dt=key.hatewords)
...
###positive sent- afinn
pos.sent<-sentiment_by(trainsentences, list(id), lexicon="afinn")
pos.sent.t<-sentiment_by(testsentenses, list(id), lexicon="afinn")
